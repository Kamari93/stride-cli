...
class ActivityService:
    ...
    def sort_activities(self, activities: list[Activity], sort_by: str, is_descending: bool = False) -> list[Activity]:
        '''Return activities sorted by the requested field.'''
        if sort_by == "date":
            return sorted(activities, key=lambda activity: activity.date, reverse=is_descending)
        if sort_by == "distance":
            return sorted(activities, key=lambda activity: activity.distance, reverse=is_descending)
        if sort_by == "duration":
            return sorted(activities, key=lambda activity: activity.duration, reverse=is_descending)
        if sort_by == "pace":
